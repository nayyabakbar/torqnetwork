...
async function getHourlyEarnings(userId, sessionId) {
  ...
  const hourlyEarnings = {
    ...
    time: new Date(),
    percentage: Number(
      ((hourlyMiningRate / constants.baseMiningRate) * 100).toFixed(2)
    ),
  };
...
async function startStaking(req, res) {
  try {
    ...
    if (session) {
      ...
      const stakingJob = createStakingJob(
        user._id,
        newStaking._id,
        amount,
        years
...
function createStakingJob(userId, stakingId, amount, years) {
  return async function () {
    try {
      const stakingDeposit = await Staking.findByIdAndUpdate(stakingId, {
        $set: { isActive: false },
      });
      if (stakingDeposit) {
        const getStakedAmount = calculateInterest(amount, years);
        const user = await User.findByIdAndUpdate(userId, {
          $inc: { availableBalance: getStakedAmount, stakingBalance: -amount },
        });
        if (!user) {
...
function calculateInterest(amount, years) {
  switch (years) {
    case 1:
      return 0.25 * amount + amount;
    case 2:
      return 0.65 * amount + amount;
    case 3:
      return 1.2 * amount + amount;
    case 4:
      return 1.9 * amount + amount;
    case 5:
      return 2.75 * amount + amount;
    default:
      return 0;
  }
}
